...
% covariance figure-----------------------------------
Figure \ref{fig:covariances} shows snapshots of the covariance between the zonal wind field and $\chi_3$ over the Northern Hemisphere, in the simulation assimilating only the angular momentum, on a tropospheric level [320 hPa; (a), (c), and (e)], and on a stratospheric level [10 hPa; (b), (d), and (f)].
...
% (a) time dependence of covariances
...
% (b) covariances are tiny rel to sampling error 
...
% (c) larger spatial scales in the stratosphere 
...
To illustrate what these covariance fields mean for the assimilation of angular momentum data, 
...
%--redundant?---Thus, while the covariance model developed by the ensemble filter has a physical basis, its high degree of sampling error can also lead to analysis increments that make the state estimate worse, all while the ensemble spread decreases.  
